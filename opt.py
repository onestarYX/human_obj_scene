...
import glob
import json
from utils.io import get_absolute_path
...
def get_ckpt_path_dir_path_from_environment_dir(environment_dir):
    with open(os.path.join(environment_dir, "choices.json"), 'r') as f:
        choices = json.load(f)
    ...
    if choices["directory"] == "":
        ...
        dir_path = os.path.join(environment_dir, "outputs/")
        os.makedirs(dir_path, exist_ok=True)
    else:
        dir_path = choices["directory"]
        os.makedirs(dir_path, exist_ok=True)
...
#                         help='scene name, used as output folder name')
    ...
    parser.add_argument('--fine_coarse_same', action='store_true', default=False)
