...
def render_to_path(path, dataset, idx, models, embeddings, config, do_grouping, label_colors, save_img=False):
    sample = dataset[idx]
    rays = sample['rays']
...
if __name__ == '__main__':
    ...
    if config.dataset_name == 'sitcom3D':
        ...
        dataset = SitcomSAMDataset(split=args.split, img_downscale=config.img_downscale_val, **kwargs)
    elif config.dataset_name == 'blender':
    ...
    models = {'coarse': nerf_coarse, 'fine': nerf_fine, 'garfield_predictor': garfield_predictor}

    sam_dict_file = Path("data/sparse_reconstruction_and_nerf_data/HIMYM-red_apartment/sam_masks/sam_results.pkl")
    with open(sam_dict_file, 'rb') as f:
        sam_dict = pickle.load(f)
    scales_stats = sam_dict['scales']
    scales_stats = torch.cat(scales_stats, dim=0)  # (all_num_masks, 1)
    garfield_predictor.get_quantile_func(scales_stats)
